...
def send_sms(client, src_addr, dst_addr, message):
    # Send SMS
...
def handle_incoming_sms(pdu, client):
    ...
    if str(source_address) != "11111":
        ...
        hashed_message = sha256_hash(decrypted_message)
        hashed_message = hashed_message + " | AFTSTC"
        
        encrypted_feedback = aes_encrypt(hashed_message, key)
        send_sms(client, pdu.destination_addr.decode(), source_address, f'$${encrypted_feedback}$$')

def receive_sms(client):
    while True:
        try:
            client.set_message_received_handler(lambda pdu: handle_incoming_sms(pdu, client))
...
def send_and_receive_sms(host, port, username, password, src_addr):
...
send_and_receive_sms(host, port, username, password, src_addr)